...
def arie_rank(X: pd.DataFrame, types: dict, weights: np.ndarray, targets: dict, gamma: float=1.0, kappa: float=0.5):
    ...
    df_rank = df_sim.sort_values("RC", ascending=False)
    df_rank["Rank"] = np.arange(1, len(df_rank)+1)
    df_rank = df_rank[["Rank", "RC", "Sim_best", "Sim_worst"]]
...
if run:
    try:
        ...
        with tab4:
            ...
            st.bar_chart(df_rank["RC"])

        
            # --- New charts: RC-ordered Pie & Line (distinct colors per Alternative) ---
            rc_df = df_rank.reset_index()[["Alternative", "RC"]]
            try:
                import altair as alt
                # Ensure RC-sorted order is respected in visuals
                alt_sort = list(rc_df["Alternative"])

                # Pie chart (RC contribution by Alternative)
                pie_chart = (
                    alt.Chart(rc_df)
                    .mark_arc(outerRadius=110)
                    .encode(
                        theta=alt.Theta("RC:Q", stack=True),
                        color=alt.Color("Alternative:N",
                                        sort=alt_sort,
                                        scale=alt.Scale(scheme="category20")),
                        tooltip=[alt.Tooltip("Alternative:N"), alt.Tooltip("RC:Q", format=".6f")]
                    )
                )
                st.altair_chart(pie_chart, use_container_width=True)

                # Line chart (RC by Alternative, in RC order)
                line_chart = (
                    alt.Chart(rc_df)
                    .mark_line(point=True)
                    .encode(
                        x=alt.X("Alternative:N", sort=alt_sort, title="Alternative (RC order)"),
                        y=alt.Y("RC:Q", title="Relative Closeness"),
                        color=alt.Color("Alternative:N",
                                        sort=alt_sort,
                                        scale=alt.Scale(scheme="category20")),
                        tooltip=[alt.Tooltip("Alternative:N"), alt.Tooltip("RC:Q", format=".6f")]
                ...
                # Fallback to Matplotlib if Altair is unavailable
                import matplotlib.pyplot as plt
                from matplotlib import cm
                labels = rc_df["Alternative"].tolist()
                values = rc_df["RC"].tolist()
                ...
                ax2.set_xlabel("Alternative (RC order)")
                ax2.set_ylabel("Relative Closeness")
